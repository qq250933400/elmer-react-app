...
import os from 'os';
import 'colors';
import { checkDir } from '../src/builder/function';
...
* @param {*} path 
*/
const check_dir = (path) => {
    path = path.replace(/\\/g, "/");
    var arr = path.split("/");
    var isWin = /:\//.test(path) ? true : false;
    var tmpDirArr = [];
...
const scanFile = (myPath, fn) => {
    if (fs.existsSync(myPath)) {
        ...
        console.log(`[err]Can not find the path:${path}`.red);
    }
};
...
const exportFiles = () => {
    const ExportResult = {};
    scanFile(desLocalPath, (parentPath,currentPath, isDirectory) => {
        if (!isDirectory) {
...
if (config.language.indexOf(config.languageStandard)<0) {
    ...
    scanFile(path.resolve(__dirname, config.tpPath), (parentPath, currentPath, isDirectory) => {
        ...
        if (isDirectory && !exists(lngPath)) {
            ...
            if (currentPath.replace(/\\/g, '/') !== tpLocalPath + '/index.js' && !isDirectory) {
